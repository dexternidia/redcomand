...
class CentrosTestigosMunicipal
{
    ...
    public function create()
    {
        ...
        {
            Error('centrosMesasMunicipal/'.$id_mesa,'Esta persona ya es responsable de un centro.');
        }
        else
        {
                ...
                if($datos_cne->municipio == $mesa->id_municipio AND $datos_cne->parroquia == $mesa->id_parroquia)
                {
                    if($testigosCount < 3)
                    {
                        ...
                        if($testigo->save())
                        {
                            ...
                            Success('centrosMesasMunicipal/'.$id_mesa,'Testigo de mesa agregado con exito.!');
                        }
                        else
                        {
                            Error('centrosMesasMunicipal/'.$id_mesa,'Error al agregar testigo.');
                        }
                    }
                    else
                    {
                        Error('centrosMesasMunicipal/'.$id_mesa,'Ya supero el limite de testigos por mesa.');
                    }
                }
                else
                {
                    Error('centrosMesasMunicipal/'.$id_mesa,'El testigo de mesa tiene que ser del mismo centro en el cual vota.');
                }
            }
            else
            {
                Error('centrosMesasMunicipal/'.$id_mesa,'No se encuentra registrado en el CNE.');
            }
            //Arr($datos_cne);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if($testigo->delete())
        {
            Success('centrosMesasMunicipal/'.$id_mesa,'Testigo de mesa borrado con exito.!');
        }
        else
        {
            Error('centrosMesasMunicipal/'.$id_mesa,'No se encuentra registrado en el CNE.');
