...
<div id="panel" class="panel panel-primary">
  ...
  <div class="panel-body">
    <form action="<?php echo baseUrlRole() ?>centrosAdmin" method="POST">
      <?php echo Token::field() ?>
      <div class="row">
        <div class="col-lg-4">
          <div class="form-group">
            <select id="municipioSelect" class="form-control" name="id_mesa" required/>
              <option>CENTROS</option>
              <option></option>
              <?php foreach ($mesas_cne as $m): ?>
              <option value="<?php echo $m->id_mesas_cne ?>"><?php echo $m->nombre ?></option>
              <?php endforeach ?>
            </select>
          </div>
        </div>
