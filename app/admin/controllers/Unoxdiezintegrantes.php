...
class UnoxdiezintegrantesAdmin
{
        ...
        if($datos_cne)
        {
            if($datos_cne->municipio == $user['id_municipio'] AND $datos_cne->parroquia == $user['id_parroquia'])
            {
                ...
                else
                {
                    ...
                    {
                        Error('unoxdiezintegrantesAdmin/create/'.$id_ubch_registro_unoxdiez,'Esta persona ya es ahijado de un 1x10.');
                    }
                    else
                    {
                        if($conteoUnoxdiez->count() > 1)
                        {
                            Error('unoxdiezAdmin/'.$id_ubch_registro_unoxdiez,'Error al crear ahijado de 1x10.');
                        }
                        else
                        {
                            ...
                            if($padrino->save())
                            {
                                Success('unoxdiezAdmin/'.$id_ubch_registro_unoxdiez,'Ahijado de 1x10 creado con exito.!');
                            }
                            else
                            {
                                Error('unoxdiezAdmin/'.$id_ubch_registro_unoxdiez,'Error al crear ahijado de 1x10.');
            ...
            {
                Error('unoxdiezintegrantesAdmin/create/'.$id_ubch_registro_unoxdiez,'No Pertenece a la misma zona del centro de votación.');
            }
        }
        else
        {
            Error('unoxdiezintegrantesAdmin/create/'.$id_ubch_registro_unoxdiez,'No se encuentra en el registro CNE.');
        }
        //View();
    }
    ...
    public function destroy($id)
    {
        ...
        if($ahijado->delete())
        {
            Success('unoxdiezAdmin/'.$id_ubch_registro_unoxdiez,'Ahijado borrado con exito.');
        }
        else
        {
            Success('unoxdiezAdmin/'.$id_ubch_registro_unoxdiez,'Erroral borrar ahijado.');
        }
    }
}